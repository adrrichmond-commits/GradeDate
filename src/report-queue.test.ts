...
describe("report queue policy", () => {
 test("allowlists roles and reasons", () => { expect(canManageReport("moderator")).toBe(false); expect(canManageReport("user")).toBe(false); expect(canManageReport("admin")).toBe(true); expect(canUseOwnerAction("moderator")).toBe(false); expect(isReportReason("harassment")).toBe(true); expect(isReportReason("private_message")).toBe(false); });
