...
export async function getAppeals(userId?:number):Promise<any[]>{const r= userId===undefined ? await sql()`SELECT id,suspension_id,user_id,status,created_at,reviewed_at,text FROM appeals ORDER BY created_at DESC` : await sql()`SELECT id,suspension_id,status,created_at,reviewed_at,text FROM appeals WHERE user_id=${userId} ORDER BY created_at DESC`; return r as any[];}
